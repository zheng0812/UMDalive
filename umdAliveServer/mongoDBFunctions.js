...
var temp = userData.email;
    ...
    mongoDBRef.collection('users').save({user: title, userData}, function(err, result){
...
*
*/
module.exports.findUser = function(emailAddr, callback){
    mongoDBRef.collection('users').find({email: emailAddr}).toArray(function(err,docs){
    if(!err){
    console.log("Found the following records");
    console.log(docs);
    callback(docs);
